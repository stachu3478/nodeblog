...
var dbutils = new require('./mods/dbutils.js').util(mongodb, fs);
	...
	switch(url){
				...
				getPost(req, (body) => {
					...
					try{
						...
						if(out == true){
							...
							res.writeHead(506, {'Content-Type': 'text/plain'});
		...
		case "/login": {
			if(req.method == "POST"){
				...
				getPost(req, (body) => {
					...
					dbutils.getUser(data.login, data.password, function(){
					...
					getPost(req, function(str){
						var data = qs.parse(str);
							...
							var onDone = () => {
								db.close();
								showProfile(req, res, function(){
									res.write("Usunięto konto oraz " + artsDeleted + " artykułów przypisanych do konta.");
								});
							};
							var artsDeleted;
							dbo.collection("articles").deleteMany({authorId: new mongo.ObjectID(data._id)}, function(err, result){
								if(err)throw err;
								db.close();
								
								artsDeleted = result.result.n;
								if(++done == 2)onDone();
							});
				...
				if(req.method == "POST" && req.session.has('userId')){
					getPost(req, function(str){
						var data = qs.parse(str);
							...
							if(err)throw err;
							dbo = db.db("blogmaster");
							dbo.collection("articles").insertOne({title: data.title, text: data.text, publishDate: data.date, authorId: new mongo.ObjectId(req.session.get('userId'))}, function(err, result){
								if(err)throw err;
								db.close();
								showProfile(req, res, function(){
		...
		default: {
			if(fs.existsSync(sciezka)){
				if(fs.lstatSync(sciezka).isDirectory()){
					...
					if(fs.existsSync(defaultFile)){
						...
						fs.readFile(defaultFile, function(err,data){
							if(data.indexOf("##articels")){
								data = data.toString().replace("##articles", getArticles().join(""));
							};
							res.write(data);
							res.end();
						});
				...
				res.write(fs.readFileSync('htdocs/err404.html'));
				res.end();
			};
		};break;
	};
